import { executeCommand, initializeWebContainer, setOutputCallback } from "../components/api";
...
function Terminal() {
    ...
    const handleCreateProject = (framework) => {
        ...
        if (framework === 'React') {
            command = `npm create -y vite@latest ${projectName} -- --template react`;
    ...
    return (
        <div className="w-1/3 h-full flex flex-col gap-2">
            ...
            <form
                className="w-full bg-[#1A1A1A] flex flex-col custom-scrollbar text-white p-4 rounded border border-zinc-600">
                <label>
                    Command:
                </label>
                <input
            ...
            <div className="w-full h-100 flex-grow bg-[#1A1A1A] flex flex-col text-white p-4 rounded border border-zinc-600 overflow-hidden">
                <pre
                    ...
                    className="w-full flex flex-col-reverse custom-scrollbar overflow-auto h-full bg-zinc-800 text-white p-2 rounded outline-none">
                    <code>
